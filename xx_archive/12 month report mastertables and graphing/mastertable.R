...
SELECT *
FROM [Y006_BBV_PID].[dbo].[12monthECDSattendances_includedsites]
...
attendees_antiHCV <- attendees_HCV %>% filter(HCVAb == "Positive")
...
write.csv(joined_table, "MasterTables_export.csv")


### tables by age and sex for population pyramids
# attendees by age and sex (all sites)
attendees_age_sex <- as.data.frame(table(attendees_all$age_group, attendees_all$Sex)) %>%
    rename(
        "age_group" = "Var1",
        "Sex" = "Var2"
    ) %>%
    filter(age_group != "Unknown Age" & Sex != "Unknown") %>%
    mutate(Sex = recode(Sex, "FEMALE" = "Female", "MALE" = "Male")) %>%
    mutate(
        proportion = round(Freq / sum(Freq), 2) * 100
    ) %>%
    mutate(proportion = case_when(
        Sex == "Female" ~ proportion * -1,
        Sex == "Male" ~ proportion
    )) %>%
    mutate(proportion2 = case_when(
        Sex == "Female" ~ proportion * -1,
        Sex == "Male" ~ proportion
    )) 

# attendees by age and sex (5 sites)
attendees_age_sex5 <- as.data.frame(table(attendees_HIV$age_group, attendees_HIV$Sex)) %>%
    rename(
        "age_group" = "Var1",
        "Sex" = "Var2"
    ) %>%
...
# blood test uptake by age and sex (all sites)
bloodtests_all <- attendees_all %>%
group_by(ECDS_bloods_any, Sex, age_group) %>%
summarise(no_obs = n())

bloodtests_age_sex <- bloodtests_all %>%
filter(age_group != "Unknown Age" & Sex != "Unknown") %>%
    mutate(Sex = recode(Sex, "FEMALE" = "Female", "MALE" = "Male")) %>%
    group_by(Sex, age_group) %>%
    mutate(
        proportion = round(no_obs / sum(no_obs), 2) * 100
    ) %>%
    mutate(proportion = case_when(
        Sex == "Female" ~ proportion * -1,
        Sex == "Male" ~ proportion
...
# blood test uptake by age and sex (5 sites)
# number of tests by age and sex over number of attendees by age and sex
bloodtests_5 <- attendees_HIV %>%
group_by(ECDS_bloods_any, Sex, age_group) %>%
summarise(no_obs = n())

bloodtests_age_sex_5 <- bloodtests_5 %>%
filter(age_group != "Unknown Age" & Sex != "Unknown") %>%
    mutate(Sex = recode(Sex, "FEMALE" = "Female", "MALE" = "Male")) %>%
    group_by(Sex, age_group) %>%
    mutate(
...
# BBV test uptake (all attendees) by age and sex (5 sites)
## HIV
hivtests_5 <- attendees_HIV %>%
group_by(HIV, Sex, age_group) %>%
summarise(no_obs = n())

hivtests_age_sex_5 <- hivtests_5 %>%
filter(age_group != "Unknown Age" & Sex != "Unknown") %>%
    mutate(Sex = recode(Sex, "FEMALE" = "Female", "MALE" = "Male")) %>%
...
## HCV
hcvtests_5 <- attendees_HCV %>%
group_by(HCV, Sex, age_group) %>%
summarise(no_obs = n())

hcvtests_age_sex_5 <- hcvtests_5 %>%
    filter(age_group != "Unknown Age" & Sex != "Unknown") %>%
    mutate(Sex = recode(Sex, "FEMALE" = "Female", "MALE" = "Male")) %>%
    group_by(age_group, Sex) %>%
    mutate(
...
## HBV
hbvtests_5 <- attendees_HBV %>%
group_by(HBV, Sex, age_group) %>%
summarise(no_obs = n())

hbvtests_age_sex_5 <- hbvtests_5 %>%
    filter(age_group != "Unknown Age" & Sex != "Unknown") %>%
    mutate(Sex = recode(Sex, "FEMALE" = "Female", "MALE" = "Male")) %>%
    group_by(age_group, Sex) %>%
    mutate(
...
# BBV test uptake (attendees with a blood test) by age and sex (5 sites)
## HIV
hivtests_5_bt <- attendees_HIV %>%
filter(ECDS_bloods_any == "Yes") %>%
group_by(HIV, ECDS_bloods_any, Sex, age_group) %>%
summarise(no_obs = n())

hivtests_age_sex_5_bt <- hivtests_5_bt %>%
filter(age_group != "Unknown Age" & Sex != "Unknown") %>%
    mutate(Sex = recode(Sex, "FEMALE" = "Female", "MALE" = "Male")) %>%
    group_by(age_group, Sex) %>%
    mutate(
...
## HCV
hcvtests_5_bt <- attendees_HCV %>%
filter(ECDS_bloods_any == "Yes") %>%
group_by(HCV, ECDS_bloods_any, Sex, age_group) %>%
summarise(no_obs = n())

hcvtests_age_sex_5_bt <- hcvtests_5_bt %>%
filter(age_group != "Unknown Age" & Sex != "Unknown") %>%
    mutate(Sex = recode(Sex, "FEMALE" = "Female", "MALE" = "Male")) %>%
    group_by(age_group, Sex) %>%
    mutate(
...
## HBV
hbvtests_5_bt <- attendees_HBV %>%
filter(ECDS_bloods_any == "Yes") %>%
group_by(ECDS_bloods_any, HBV, Sex, age_group) %>%
summarise(no_obs = n())

hbvtests_age_sex_5_bt <- hbvtests_5_bt %>%
    filter(age_group != "Unknown Age" & Sex != "Unknown") %>%
    mutate(Sex = recode(Sex, "FEMALE" = "Female", "MALE" = "Male")) %>%
    group_by(age_group, Sex) %>%
    mutate(
...
write.csv(joined_table_hiv, "MasterTables_export.csv")
