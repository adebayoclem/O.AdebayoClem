...
proportion_diagnosed_HIV <- function(data, demographic_var) {
  if (nrow(data) == 0) return(data.frame())
  result <- data %>%
    group_by(!!sym(demographic_var)) %>%
    summarise(
      Diagnosed_Attendees = sum(new_diagnosis == "TRUE", na.rm = TRUE),
      Blood_Tested_Attendees = sum(ECDS_bloods_any == "Yes" & HIV == "Yes", na.rm = TRUE),
